...
impl MmCif {
	...
	pub fn read_cif_info( &mut self, arg_i : &Vec<&str> /*&String*/ ) {
	...
	pub fn define_category_order( &mut self ) {
		...
		self.category_order = vec![ 0; /*13*/ 9 ];
		...
		for i in 0 .. ( self.category_list ).len() {
			...
			else if ( self.category_list )[ i ].starts_with( "_atom_site.Cartn_x"       ) { ( self.category_order )[ /*10*/ 6 ] = i; }
			else if ( self.category_list )[ i ].starts_with( "_atom_site.Cartn_y"       ) { ( self.category_order )[ /*11*/ 7 ] = i; }
			else if ( self.category_list )[ i ].starts_with( "_atom_site.Cartn_z"       ) { ( self.category_order )[ /*12*/ 8 ] = i; }
		}
	...
	pub fn set_data_vector( &mut self ) {
		...
		for line in ( self.atom_site_data ).iter() {
			...
			( self.cartn_x       ).push( compornents[ ( self.category_order )[ /*10*/ 6 ] ].to_string() );
			( self.cartn_y       ).push( compornents[ ( self.category_order )[ /*11*/ 7 ] ].to_string() );
			( self.cartn_z       ).push( compornents[ ( self.category_order )[ /*12*/ 8 ] ].to_string() );
		}
	}
	...
	pub fn remove_alt_id( &mut self ) {
		...
		if ( self.label_atom_id ).len() != ( self.group_pdb ).len() { error::error_bomb( "len_label_atom_id_group_pdb_not_same" ); process::exit( 1 ); }
		if ( self.label_alt_id  ).len() != ( self.group_pdb ).len() { error::error_bomb( "len_label_alt_id_group_pdb_not_same"  ); process::exit( 1 ); }
		if ( self.label_comp_id ).len() != ( self.group_pdb ).len() { error::error_bomb( "len_label_comp_id_group_pdb_not_same" ); process::exit( 1 ); }
		if ( self.label_asym_id ).len() != ( self.group_pdb ).len() { error::error_bomb( "len_label_asym_id_group_pdb_not_same" ); process::exit( 1 ); }
		if ( self.label_seq_id  ).len() != ( self.group_pdb ).len() { error::error_bomb( "len_label_seq_id_group_pdb_not_same"  ); process::exit( 1 ); }
		//if ( self.auth_comp_id  ).len() != ( self.group_pdb ).len() { error::error_bomb( "len_auth_comp_id_group_pdb_not_same"  ); process::exit( 1 ); }
		//if ( self.auth_asym_id  ).len() != ( self.group_pdb ).len() { error::error_bomb( "len_auth_asym_id_group_pdb_not_same"  ); process::exit( 1 ); }
		//if ( self.auth_atom_id  ).len() != ( self.group_pdb ).len() { error::error_bomb( "len_auth_atom_id_group_pdb_not_same"  ); process::exit( 1 ); }
		//if ( self.auth_seq_id   ).len() != ( self.group_pdb ).len() { error::error_bomb( "len_auth_seq_id_group_pdb_not_same"   ); process::exit( 1 ); }
		if ( self.cartn_x       ).len() != ( self.group_pdb ).len() { error::error_bomb( "len_cartn_x_group_pdb_not_same"       ); process::exit( 1 ); }
		if ( self.cartn_y       ).len() != ( self.group_pdb ).len() { error::error_bomb( "len_cartn_y_group_pdb_not_same"       ); process::exit( 1 ); }
		if ( self.cartn_z       ).len() != ( self.group_pdb ).len() { error::error_bomb( "len_cartn_z_group_pdb_not_same"       ); process::exit( 1 ); }
	...
	pub fn get_res_coord_info( &self, arg_c : &String ) -> Vec<usize> {
...
fn get_coord_ca(
	...
	let mut target_data_index : Vec<usize> = Vec::new();
	...
	if !( *chain_id_vec ).contains( arg_c ) { error::error_bomb( "chainid_not_contained" ); process::exit( 1 ); } 
